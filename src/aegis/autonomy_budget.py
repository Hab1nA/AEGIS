...
AUTONOMY_ROLE_CALLS: Mapping[str, int] = {
    "warrior": 121,
    "judge": 50,
    "prosecutor": 50,
}
AUTONOMY_ATTEMPTS_PER_CALL = 3
AUTONOMY_PROMPT_RESERVE_BYTES = 16_384
AUTONOMY_MIN_AGENT_STEPS = 20
AUTONOMY_MIN_OUTPUT_TOKENS = 4_096
AUTONOMY_MIN_REQUESTS = sum(AUTONOMY_ROLE_CALLS.values()) * AUTONOMY_ATTEMPTS_PER_CALL
AUTONOMY_RECOMMENDED_TOTAL_TOKENS = 14_000_000
AUTONOMY_RECOMMENDED_MAX_REQUESTS = 800
AUTONOMY_RECOMMENDED_WALL_TIME_SECONDS = 28_800
...
V2_RECOMMENDED_TOTAL_TOKENS = 8_000_000
...
def autonomy_budget_check(
    *,
    ...
    """Prove capacity for the shortest complete evidence chain and relay retries."""

    if set(role_shares) != set(AUTONOMY_ROLE_CALLS) or set(max_output_tokens) != set(
        AUTONOMY_ROLE_CALLS
    ):
        raise ValueError("autonomy budget roles must be warrior, judge, and prosecutor")
    role_required = {
        role: calls
        * AUTONOMY_ATTEMPTS_PER_CALL
        * (AUTONOMY_PROMPT_RESERVE_BYTES + max_output_tokens[role])
        for role, calls in AUTONOMY_ROLE_CALLS.items()
    }
    ...
    if max_requests < AUTONOMY_MIN_REQUESTS:
        failures.append(f"requests {max_requests} < {AUTONOMY_MIN_REQUESTS}")
