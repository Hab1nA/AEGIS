...
| Warrior 进化反馈 | `evolution_direction` 注入求解臂 context：最近被拒候选原因（含 per-seed 离散度，事件流回放）、种群格子分布与多样性报告——提案动机来自上一代证据而非盲猜 | `cycle_ports.py`、`evolution/registry.py`、`evolution/population.py` | `test_cycle_ports.py`、`test_evolution_registry.py` |
| Judge 出题回流 | `curriculum_direction` 增补上一周期 `prior_authoring_rejections`（含难度门拒绝模式）与 `prior_hypothesis_coverage` 核算，出题质量跨代复利 | `cycle_ports.py` | `test_cycle_ports.py` |
| 种子轮换与扩种 | 评测种子 = 锚 0（保主解复用）+ 按（campaign, cycle, slot）派生的轮换槽位；`evaluation_seed_count ∈ [2,4]` 控制面配置（supersede control-core 候选值）；fresh 均值落在噪声带（正向未达门槛）时自动追加一颗种子重裁决（至多一次），gate report 附 advisory bootstrap 区间（不参与裁决） | `cycle_ports.py`、`evaluation/promotion.py`、`attribution/candidate_gate.py`、`evolution/control_core.py` | `test_cycle_ports.py`、`test_candidate_gate.py`、`test_evaluation_promotion.py`、`test_control_core.py` |
| per-surface 回滚 | 检察官回滚令按面分派：harness-code 走 git 回退；workflow/subject/plugin/environment 翻转 registry 至父代并重物化 role-version manifest（回滚 = 重新激活上一 champion），执行结果持久化 | `cycle_ports.py`、`evolution/registry.py`、`evolution/runtime.py` | `test_cycle_ports.py`、`test_evolution_harness.py` |
| 候选试用期 | 非 harness 面 champion 的观察窗口（`candidate_probation_cycles ∈ [0,16]`，默认 2 次有效观察）：每周期以父代绑定跑一次 seed=0 配对臂，相对主解回归越过非劣裕度即写 breach 事件（注入检察官 audit context 的 `probation_breaches`，引导 `aegis.order_rollback`）；主解未覆盖评测 cohort 的周期跳过（防 anchor 回填制造虚假回归）；窗口期满无 breach 写毕业事件。状态从事件流重建 | `cycle_ports.py`、`config.py` | `test_cycle_ports.py` |
| Canary 加厚 | harness-code 金丝雀按 `ChangeManifest.files` 改动根映射控制面拥有的确定性测试子集（默认单文件为下限，上限 6 文件，双臂同集）；显式 `harness_canary_command` 仍整体覆盖；WSL 主路径不经此 runner | `evolution/harness.py` | `test_evolution_harness.py` |
| Skill 通道接线 | CLI 构造 `SkillRegistry`（`skills.sqlite3`），`skill.list/stage` 成为真实可用动作（协议不变） | `cli.py`、`skill_registry.py` | `test_cli.py` |
| 归因账本镜像 | `_append_arm` 在 jsonl 之外镜像 `attribution-arm` 工件 + `attribution_arm_recorded_v1` 事件，data-dir 重建后归因可重放 | `cycle_ports.py` | `test_cycle_ports.py` |
...
| 受限联网 | 运行时沙箱一律 `--network=none`（命令协议仅接受 `none`，无域名白名单运行时）；环境构建为离线双构建，依赖为 digest-pinned HTTPS 制品经隔离下载 | `config.py`、`sandbox/agent.py` | `test_config.py`、`test_sandbox_agent.py` |
...
| 元进化边界 | `meta_evolution_enabled` 显式授权后，Warrior 才可进化演化控制机器（`evolution/registry.py`、`evolution/consumer.py`、授权规则本体 `evolution/surfaces.py`、金丝雀 `evolution/harness.py`，以及恢复链 `cycle_recovery.py`、`repair_runtime.py`）；沙箱/发布/配置/评测/归因边界与用量记账（`usage_accounting.py`，冻结于任何可进化根之外）永不开禁；预检含 `meta_evolution_boundary` 门禁 | `config.py`、`evolution/surfaces.py`、`evolution/harness.py`、`cli.py` | `test_evolution_harness.py::MetaEvolutionTests` |
| 用量记账冻结 | `TokenUsage` 构造与 relay usage 提取唯一收录于 `usage_accounting.py`（不在任何可进化根内）；不可能数字（输出超出预留预算、reasoning 超出输出等）打 `anomalies` 并降级 `verified=false`；异常经 `_usage_summary` 进角色证据、经 `usage_anomalies` 进检察官审计上下文 | `usage_accounting.py`、`gateway/client.py`、`cycle_ports.py` | `test_usage_accounting.py` |
| Harness mirror 同步 | `aegis harness-sync <campaign_id>` 把 WSL source mirror 刷新到配置钉住的 `harness_source_ref`（fetch 公共源 + rev-parse 校验，fail-closed），消除宿主仓库与 distro mirror 的漂移；agent 侧为 `sync_mirror` 操作，幂等回执 | `cli.py`、`evolution/harness_backend.py`、`evolution/wsl_harness_agent.py` | `test_harness_backend.py` |
