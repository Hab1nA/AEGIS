...
| 候选试用期 | 非 harness 面激活后 K 个周期（`candidate_probation_cycles ∈ [0,16]`，默认 2）内每周期以父代绑定跑一次配对臂，相对主解回归越过非劣裕度即写 breach 事件，期满达标写毕业事件；状态从事件流重建 | `cycle_ports.py`、`config.py` | `test_cycle_ports.py` |
